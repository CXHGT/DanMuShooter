...
BilibiliGetCodeDialog::BilibiliGetCodeDialog(QWidget *parent)
	: QDialog(parent), ui(new Ui::BilibiliGetCodeDialog)
{
	ui->setupUi(this);
...
void BilibiliGetCodeDialog::setIsSaveCode(bool isSave)
{
    ui->radioButton->setChecked(isSave);
}
void BilibiliGetCodeDialog::setCodeText(const std::string &code)
{
    ui->codeLineEdit->setText(code.c_str());
}
...
void BilibiliGetCodeDialog::mousePressEvent(QMouseEvent *event)
{
	if (event->button() == Qt::LeftButton)
	{
		...
		{
			m_bDragging = true;
			m_poStartPosition = event->globalPosition().toPoint();
...
void BilibiliGetCodeDialog::mouseMoveEvent(QMouseEvent *event)
{
	if (event->buttons() & Qt::LeftButton) //只响应鼠标所见
	{
		...
		{
			// delta 相对偏移量,
			QPoint delta = event->globalPosition().toPoint() - m_poStartPosition;
			// 新位置：窗体原始位置  + 偏移量
